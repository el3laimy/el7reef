...
class TournamentPermissionService extends GetxService {
  ...
  /// هل يملك المستخدم الصلاحية لإدارة الفرق الضيفة؟
  bool canManageGuestTeams(Tournament tournament, String userId) {
    if (tournament.organizerId == userId) return true;
  ...
  /// هل يملك المستخدم الصلاحية لإدارة roster الفرق الضيفة؟
  bool canManageGuestRoster(Tournament tournament, String userId) {
    if (tournament.organizerId == userId) return true;
  ...
  /// هل يملك المستخدم الصلاحية لإصدار روابط claim للاعبي أو فرق الضيف؟
  bool canIssueGuestClaims(Tournament tournament, String userId) {
    if (tournament.organizerId == userId) return true;
  ...
  TournamentAssistantRole? _getActiveRole(
    Tournament tournament,
    String userId,
  ) {
    try {
      final assistant = tournament.assistants.firstWhere(
        (a) => a.userId == userId,
      );

...
class MatchInvitationRepositoryImpl implements MatchInvitationRepository {
  ...
  MatchInvitationRepositoryImpl({FirebaseFirestore? firestore})
  ...
  Future<MatchInvitation?> getInvitation(String id) async {
    return FirebaseErrorHandler.guard(() async {
      ...
      if (doc.exists && doc.data() != null) {
        return MatchInvitationModel.fromJson(
  ...
  @override
  Future<List<MatchInvitation>> getPendingInvitationsForUser(
    String userId,
  ) async {
    return FirebaseErrorHandler.guard(() async {
      ...
      return snapshot.docs
          .map(
            (doc) => MatchInvitationModel.fromJson(
              doc.data() as Map<String, dynamic>,
              doc.id,
            ),
          )

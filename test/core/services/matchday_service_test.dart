...
void main() {
  group('MatchdayService', () {
    ...
    setUp(() {
      ...
      substitutionRepository = MatchSubstitutionRepositoryImpl(firestore: firestore);
      now = DateTime(2026, 4, 16, 20);
    });

    test('checks in a registered team and locks a valid lineup snapshot', () async {
      ...
      final lockResult = await service.lockRegisteredTeamLineup(
        ...
        teamId: 'team-1',
        actorId: 'owner-1',
        starterMembershipIds: memberships.take(5).map((entry) => entry.id).toList(),
      ...
      expect(
        attendances.where((entry) => entry.startedMatch && entry.currentlyOnPitch),
        hasLength(5),
      );
      expect(
        attendances.where((entry) => entry.played && entry.firstEnteredMinute == 0),
        hasLength(5),
      );
      expect(attendances.singleWhere((entry) => entry.teamMembershipId == 'membership-6').played,
          isFalse);
      expect(
        attendances.singleWhere((entry) => entry.teamMembershipId == 'membership-6').includedInLockedLineup,
        isTrue,
      );
    });

    test('rejects lineup lock when starter count does not match tournament size',
        () async {
      await _seedTournament(tournamentRepository, now);
      final memberships = await _seedRegisteredTeam(
        teamRepository: teamRepository,
      ...
      expect(
        () => service.lockRegisteredTeamLineup(
          ...
          starterMembershipIds:
              memberships.take(4).map((entry) => entry.id).toList(),
    ...
    test('re-running lineup lock returns the existing snapshot idempotently',
        () async {
      await _seedTournament(tournamentRepository, now);
      final memberships = await _seedRegisteredTeam(
        teamRepository: teamRepository,
    ...
    test('records substitutions and updates played truth for registered teams',
        () async {
      await _seedTournament(tournamentRepository, now);
      final memberships = await _seedRegisteredTeam(
        teamRepository: teamRepository,
      ...
      final substitutionResult = await service.recordRegisteredTeamSubstitution(
        matchId: 'match-4',
        teamId: 'team-1',
        actorId: 'owner-1',
    ...
    test('organizer can check in and lock a guest team lineup from attendance truth',
        () async {
      await _seedTournament(tournamentRepository, now);
      ...
      final lockResult = await service.lockGuestTeamLineup(
        ...
        guestTeamId: 'guest-team-1',
        actorId: 'organizer-1',
        starterGuestPlayerIds: guestPlayers.take(5).map((entry) => entry.id).toList(),
      ...
      expect(attendances.where((entry) => entry.currentlyOnPitch), hasLength(5));
    });
  });
}
